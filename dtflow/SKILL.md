...
- **全量筛选/搜索**：`f` where、`/` 子串搜索 —— 均**扫描整个文件**(worker 线程，带进度，`Esc` 取消)，命中的全局行号聚成可分页「子集」浏览；状态栏显示「命中 M/N (占比%)」；`r` 清除子集回到全量。完整分布统计(直方图/分位数/value_counts)用 `dt stats`/`dt token-stats`
  - where 语法：`列名 运算符 值`，列名取**表头所见**（派生列 `chars`/`turns`/`roles` 按显示值比较；元数据列/深层路径 `source`、`messages.#>=2` 当字段路径）。运算符 `> >= < <= == != =`。多列 **`and`/`or` 组合**（and 优先级高于 or），如 `turns>=6 and chars<2000`
- **列值勾选筛选** `F` 或**点列头**（Excel AutoFilter 式）：全量列出该列唯一值+频次 → 勾选保留哪些 → 子集。适合类别列(source/roles/label)；高基数列(>300 唯一值)提示改用 `f`。作用于当前浏览序列，可与 `f` 叠加
