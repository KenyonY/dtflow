...
- **大文件窗口化浏览**：JSONL/NDJSON 普通打开只索引并加载当前窗口（`--cap`，默认 1 万行），不等待全文件扫描；`]`/`[` 翻下/上一窗口，正向索引按需延伸。总行数在读到文件末尾前显示为待定。`:` 正数跳行只扫描到目标窗口；`G`、负数跳行和全量操作在可取消的后台任务中补全索引。`--offset=N` 从第 N 行打开（0-based，需要扫描之前的内容）。`#` 列显示全局行号；已索引区域通过字节偏移直接读取
...
- 关键键：`j/k` 选行 · `]`/`[` 翻窗口 · `:` 跳行 · `n/N` 详情逐条消息 · `*` 下一命中 · `s` 全量排序 · `S` 列快照 · `/` 全量搜索 · `f` 全量where筛选(可叠加) · `F` 列值勾选 · `c` 选列 · `w` 导出 · `C` 复制命令 · `y`/`v` 复制样本 · `Enter` 放大 · `r` 清全部约束 · `?` 帮助 · `q` 退出
