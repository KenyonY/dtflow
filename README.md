...
>
> **按内容包含筛选**：`first_user~=退款`（派生列，匹配**全文**而非表格里那 160 字预览）、`source~=alpaca`、`messages[0].content~=报错`（深层路径）、`messages[*].content:join~=关键词`（搜整段对话，`:join` 不可省——不加时 `[*]` 只取第一个元素）。`~=`/`/`/值面板搜索框三个入口都不区分大小写；要区分用 `==`。
