...
| `/` `f` | **全量搜索/筛选**：`/` 子串（搜全文，非表格可见前缀）、`f` where——扫描整个文件（带进度，`Esc` 取消），命中聚成可分页子集；`r` 清除子集 |
...
**筛选是全量的**：`f`/`/`/`F` 扫描整个文件而非仅当前窗口，命中的全局行号聚成子集分页浏览，状态栏显示「命中 M/N (占比%)」，子集内可 `y`/`v` 复制导出。`F`/点列头是 Excel 式「看唯一值勾选」（适合类别列 `source`/`roles`；面板顶部搜索框可按子串过滤候选值，有搜索词时「全选」= 只保留匹配项，于是「某列包含某子串」= 打字 → 全选 → `Enter`。唯一值接近行数的列会提示改用 `f` 的 `~=`），`f` 是条件表达式——两者都作用于当前浏览序列，可叠加。查看数据时的即时筛选归 view；完整分布统计（直方图/分位数/value_counts/token）归 `dt stats` / `dt token-stats`。
