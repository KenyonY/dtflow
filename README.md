...
| 详情区拖选 + `Ctrl+c` | **鼠标拖选任意文本**：按住左键拖出高亮（所见即所选，自动换行处不错位），`Ctrl+c` 复制走并清除选区；双击选整个字段块，三击选整屏详情，点一下或 `Esc` 清除。复制走 OSC52 + 本地 `wl-copy`/`xclip`/`xsel` 双通道（SSH/tmux 下也能进本机剪贴板）。表格区的拖拽另有语义（改列宽/选行），整条样本用 `y` |
